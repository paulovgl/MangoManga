...
class AutorListagem extends Component{
  ...
  async getData(){
    ...
    let rows = [];   
    await Api.getShowAutor().then(res => {
  ...
  renderData(){
    if(this.state.data){
      ...
      actions={{ actions: ['edit', 'remove', 'create'], route: 'author' }}  />
      )
    }
    else{
      return (
  ...
  render(){
      return (
        <Fragment>
          <Main>
            <MMCardView title='Listagem dos Autores'>
                {this.renderData()}
             </MMCardView>
          </Main>
        </Fragment>          

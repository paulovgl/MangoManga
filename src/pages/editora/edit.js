...
class EditEditora extends Component {
  ...
  listenSubmit= event => {
    ...
    Api.updateEditora(id, this.state).then(
      res => {        
        if(res.status === 'success'){
          console.log(res)
  ...
  render (){
    return (
      ...
      <Main>
        ...
        <MMCardView title='Atualizar Editora' >
          ...
          <MDBRow>
            ...
            <MDBCol md='12' lg='6'>              
              <MMInput value={this.state.url} type='url' name='url' label='Página da Editora/URL de informação' onChange={this.listinput}/>
            </MDBCol>

...
class EditoraListagem extends Component{
  ...
  async getData(){
    ...
    let rows = [];   
    await Api.getShowEditora().then(res => {
  ...
  renderData(){
    if(this.state.data){
      ...
      actions={{ actions: ['edit', 'remove', 'create'], route: 'editora' }}  />
      )
    }
    else{
      return (
  ...
  render(){
      return (
        <Fragment>
          <Main>
            <MMCardView title='Listagem das Editoras'>
                {this.renderData()}
             </MMCardView>
          </Main>
        </Fragment>          
...
export default EditoraListagem
